...
async def run_benchmark(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results_filename = config.get("results_filename", "evaluation_results.json")
    save_summary = config.get("save_summary", True)
    summary_filename = config.get("summary_filename", "result.json")
    save_report = config.get("save_report", True)
    report_filename = config.get("report_filename", "report.html")
